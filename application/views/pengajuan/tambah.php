                    ...
                    <div class="card-body">
                        <form id="formPengajuan" action="<?= base_url('pengajuan/simpan') ?>" method="post">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control" id="nama" name="nama" placeholder="Masukkan Nama" value="<?= isset($nama_user) ? $nama_user : set_value('nama') ?>" required readonly>
...
$(document).ready(function(){
    // Dependent dropdown untuk jenis pengajuan -> nama barang
    $('#jenis_pengajuan').change(function(){
        var status = $(this).val();
        $('#nama_barang').html('<option value="">-- Pilih Nama Barang --</option>');
        $('#id_barang').html('<option value="">-- Pilih ID Barang --</option>');
        if(status != ''){
            $.ajax({
                url: "<?= base_url('pengajuan/get_nama_barang_by_status') ?>",
                method: "POST",
                data: {status: status},
                dataType: "json",
                success: function(data){
                    $.each(data, function(i, nama_barang){
                        $('#nama_barang').append('<option value="'+nama_barang+'">'+nama_barang+'</option>');
                    });
                }
            });
        }
    ...
    // Dependent dropdown untuk nama barang -> id barang
